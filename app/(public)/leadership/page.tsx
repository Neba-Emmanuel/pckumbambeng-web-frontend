...
export default function LeadershipPage() {
  return (
    <div>
          ...
          <div className="grid items-center gap-8 lg:grid-cols-12">
            <div className="lg:col-span-7">
              ...
              <p className="mt-5 max-w-2xl text-base text-ink-variant sm:text-lg">
                Meet the dedicated men and women guiding our church in faith,
                fellowship, and compassionate service in Kumba-Mbeng.
              </p>
            </div>
      ...
      <nav aria-label="Leadership sections" className="mx-auto flex max-w-7xl flex-wrap gap-3 px-4 pt-8 sm:px-6 lg:px-8">
        {leadershipSections.map((section) => (
          <a key={section.id} href={`#${section.id}`} className="inline-flex items-center gap-2 rounded-full border border-navy-100 bg-white px-4 py-3 text-sm font-semibold text-navy-800 hover:bg-navy-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-navy-700">
      ...
      {leadershipSections.map((section) => (
        <section key={section.id} id={section.id} aria-labelledby={`${section.id}-heading`} className="mx-auto max-w-7xl scroll-mt-32 px-4 py-12 sm:px-6 lg:px-8">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-gold-700">{section.overline}</p>
              <h2 id={`${section.id}-heading`} className="mt-2 text-2xl font-bold text-navy-900 sm:text-3xl">{section.title}</h2>
      ...
      <section id="pastoral-care" aria-labelledby="pastoral-care-heading" className="scroll-mt-32 bg-surface-low px-4 py-12 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl overflow-hidden rounded-xl bg-white shadow-sm md:grid-cols-2">
          <div className="bg-navy-900 p-6 text-white sm:p-8">
            ...
            <p className="mt-4 text-xs font-bold uppercase tracking-widest text-gold-200">Pastoral presence</p>
            <h2 id="pastoral-care-heading" className="mt-3 text-2xl font-bold sm:text-3xl">Counseling &amp; Pastoral Care</h2>
            <p className="mt-4 text-navy-100">Find support through prayer, spiritual guidance, family concerns, and times of bereavement.</p>
          </div>
